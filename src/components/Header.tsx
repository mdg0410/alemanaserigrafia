...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      <a href="#main-content" className="skip-link">
      ...
      </a>      <nav className="container mx-auto px-4 sm:px-6" aria-label="Navegación principal">
        <div className="flex items-center justify-between">
            ...
            <a href="#hero" aria-label="Inicio - Alemana de Serigrafía">
              <img 
                ...
                className={`transition-all duration-500 object-contain
                  ${isScrolled ? 'h-10 sm:h-16' : 'h-14 sm:h-24'} 
                  w-auto max-w-[75%] sm:max-w-[85%] md:max-w-[320px]`}
                style={{
          ...
          </div>          {/* Botón de menú móvil con mayor accesibilidad */}
          <button 
            className="md:hidden p-3 mr-1 rounded-lg hover:bg-white/10 transition-colors
          ...
          >
            <div className="w-6 h-6 relative flex items-center justify-center">
              ...
              <span className={`absolute h-0.5 w-full bg-white top-2.5 transition-opacity duration-300 
...
const MobileNavLink = ({ href, onClick, children }: { href: string; onClick: () => void; children: React.ReactNode }) => (
  <a 
    href={href}
    onClick={onClick}
    className="flex items-center text-white hover:text-secondary transition-colors duration-300 text-base font-medium
      py-2.5 px-4 mx-1 border-l-3 border-transparent hover:border-secondary rounded-r-lg
